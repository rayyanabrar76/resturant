...
export default function Header({ locale }: { locale: string }) {
  ...
  useEffect(() => {
    if (!isHomePage) { setIsScrolled(true); setIsVisible(true); return; }
        ...
        <div className="hidden md:flex items-center gap-9">
          {navLinks.map((link) => {
            ...
            return (
              ...
              >
                <span style={{ ...NAV_LINK_STYLE, color: isActive ? '#c17f3b' : 'rgba(247,242,235,0.6)' }}>
                  {link.name}
                </span>
              </Link>
            );
                  ...
                  >
                    ...
                    >
                      <div
                        ...
                        style={{ borderBottom: index < navLinks.length - 1 ? '1px solid rgba(247,242,235,0.07)' : 'none' }}
                      >
                        <span style={{ ...NAV_LINK_STYLE, fontSize: '12px', color: isActive ? '#c17f3b' : 'rgba(247,242,235,0.65)' }}>
                          {link.name}
                        </span>
                        <ArrowRight size={13} style={{ color: isActive ? '#c17f3b' : 'rgba(247,242,235,0.18)' }} />
                      </div>
                    </Link>
                  </motion.div>
                );
